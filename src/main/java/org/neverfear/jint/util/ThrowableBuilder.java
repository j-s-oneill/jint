 ...
 * 
 * ThrowableBuilder<WantedType> builder = new ThrowableBuilder<>(ThrowableFactory<WantedType>());
 * builder.add(SomeUnrelatedType)
 * builder.throwIfSet();
 */
public class ThrowableBuilder<T extends Throwable> {
	...
	public void throwIfSet() throws T {
		final T throwable = this.firstException;
		if (throwable != null) {
